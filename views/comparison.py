...
def render():
    render_header(
        "🔀",
        "Comparison Center",
        "Select two agreements to analyze structural differences, clause variations, and potential "
        "vulnerabilities between them. This module keeps its own document selection and never changes "
        "your globally active document.",
        badge="Agent 10"
    )
    ...
    if len(documents) < 2:
        st.info("Please upload at least two documents to use Comparison Center.")
        return

    doc_options = {doc['id']: doc['name'] for doc in documents}

    st.markdown("##### Which documents would you like to compare?")
    col1, col2 = st.columns(2)
    with col1:
        doc_a_id = st.selectbox(
            "Document 1:", options=list(doc_options.keys()), index=0,
            format_func=lambda x: doc_options[x], key="comparison_doc_a_id",
        )
    with col2:
        default_idx = 1 if len(doc_options) > 1 else 0
        doc_b_id = st.selectbox(
            "Document 2:", options=list(doc_options.keys()), index=default_idx,
            format_func=lambda x: doc_options[x], key="comparison_doc_b_id",
        )

    if doc_a_id == doc_b_id:
        st.warning("⚠️ Please select two different documents to compare.")
        return
            ...
            try:
                ...
                for c_type in all_classes:
                    ...
                    with st.expander(f"Type: {c_type}"):
                        ...
                        with col_a:
                            st.markdown(f"**Document 1 ({doc_a_name}):**")
                            st.info(text_a)
                        with col_b:
                            st.markdown(f"**Document 2 ({doc_b_name}):**")
                            st.info(text_b)
