...
export function FlaggedClauseCard({ clause }: { clause: RiskyClause }) {
  ...
  const confidenceDisplay = clause.confidence_score !== null ? `${clause.confidence_score.toFixed(2)} Confidence` : "—";
  ...
  return (
    <Box sx={{ border: "1px solid", borderColor: "divider", borderRadius: 3, p: 2, mb: 2 }}>
      ...
      <Stack direction="row" sx={{ justifyContent: "space-between", alignItems: "flex-start", mb: 1.5 }}>
        ...
        <Badge label={`${clause.risk_level.toUpperCase()} RISK`} color={borderColor} />
      </Stack>
      ...
      <Grid container spacing={1.5} sx={{ mb: 1.5 }}>
        ...
        <Grid size={4}>
          <MiniCard label="Importance" value={clause.importance_category ?? "—"} icon="📈" />
        </Grid>
        <Grid size={4}>
          <MiniCard label="Confidence" value={confidenceDisplay} icon="🎯" />
        </Grid>
      </Grid>
      ...
      >
        {fullText || "No text extracted for this clause."}
      </Typography>
      {isLong && (
        <Box
          component="button"
        ...
        >
          {fullTextOpen ? "Hide Full Clause" : "View Full Clause"}
        </Box>
      )}

      <Accordion sx={{ mt: 1.5 }} defaultExpanded disableGutters>
        <AccordionSummary>🧠 Why This Clause Is Risky</AccordionSummary>
        <AccordionDetails>
          {bullets.length > 0 ? (
            ...
            <Typography variant="caption" sx={{ opacity: 0.6 }}>
              No explanation recorded for this clause yet.
            </Typography>
          )}
        </AccordionDetails>
      </Accordion>
